...
const extraIds = (process.env.EXTRA_MEDIA_IDS || "").split(",").map(v => v.trim()).filter(Boolean);
const allowedIds = new Set([...BUILT_IN_IDS, ...extraIds]);
const origin = (process.env.GOOGLE_DRIVE_DOWNLOAD_ORIGIN || "https://drive.usercontent.google.com").replace(/\/$/, "");
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const upstream = await fetch(`${origin}/download?export=download&confirm=t&id=${encodeURIComponent(id)}`, { headers, redirect: 'follow' });
    ...
    res.setHeader('Cache-Control', `public, s-maxage=${cacheSeconds}, max-age=${cacheSeconds}`);
