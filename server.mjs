...
const allowedIds = new Set(
  [...appSource.matchAll(/\["([\w-]{10,})",/g)].map((m) => m[1]),
);
const driveOrigin = (
...
const upstreamUrl = (id) =>
  `${driveOrigin}/download?export=download&confirm=t&id=${id}`;
const mime = {
...
async function proxyMedia(req, res, id) {
  if (!allowedIds.has(id)) {
    res.writeHead(404);
    return res.end("Unknown media");
  }
  try {
    const headers = { "user-agent": "Mozilla/5.0 MCK-Local-Player/1.0" };
    if (req.headers.range) headers.range = req.headers.range;
    const upstream = await fetch(upstreamUrl(id), {
      headers,
      redirect: "follow",
    });
    const output = {};
    ...
    res.writeHead(502);
    res.end("Cannot connect to Google Drive");
  }
}
